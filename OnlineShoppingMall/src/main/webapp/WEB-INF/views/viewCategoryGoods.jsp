    ...
    %>
    	...
    	%>
	    	<c:forEach items="${sessionScope.goodsList}" var="goods">
		    	<div style="height: 200px; width: 100%;">
		    		...
		    		<div class="righttt">
			    		...
			    		<p class="goodsDescription">${goods.description}</p><br/><br/>
			    		<span class="goodsInfo">月售&nbsp;:&nbsp;${goods.numberOfSales}件</span>
			    		<span class="goodsInfo">成交&nbsp;:&nbsp;${goods.numberOfKnockdown}单</span>
			    		<span class="addToCart"><a href="viewGoods?goodsId=${goods.goodsId}&userId=<%=user.getUserId()%>"><button class="btn btn-primary"><i class="glyphicon glyphicon-shopping-cart"></i>&nbsp;&nbsp;加入购物车</button></a></span><br/><br/>
		    		</div>
	    	...
	    	<span class="pageIndexes">
		    	<span class="pageIndex"><a href="viewCategory?currentPageNumber=<%=previousPageNumber%>&category=<%=category%>">上一页</a></span>
		    	<c:forEach items="${sessionScope.pageIndex}" var="pageNumber">
		    		<span class="pageIndex"><a href="viewCategory?currentPageNumber=${pageNumber}&category=<%=category%>">${pageNumber}</a></span>
		    	</c:forEach>
		    	<span class="pageIndex"><a href="viewCategory?currentPageNumber=<%=nextPageNumber%>&category=<%=category%>">下一页</a></span>
    		</span>
    	<%
    		}else {
    	%>
